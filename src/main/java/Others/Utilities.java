...
public class Utilities {
	...
	public static BufferedImage parallelDecoder2(ArrayList<Byte> encoded, int processors) {
		...
		for(int c : header.getBlockSizes()) {
			...
			Double[] probs = header.getProbs(blockNumber);
			// add the task
			executor.submit(new ParallelDecoder2(blockBytes, probs, blockNumber, header, null, decodificada));
			start += c;
			blockNumber++;
		}				
		executor.shutdown();
	...
	public static BufferedImage writeImage(ArrayList<Integer> decoded, Header header) {
		Integer[] decodedA = new Integer[decoded.size()];
		for(int i = 0; i < decoded.size(); i++) {
			decodedA[i] = decoded.get(i);
		}
